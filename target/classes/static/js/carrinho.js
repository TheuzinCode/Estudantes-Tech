(function(){
  ...
  function formatBRL(n){
    try { return (new Intl.NumberFormat('pt-BR', { minimumFractionDigits: 2, maximumFractionDigits: 2 })).format(n); }
  ...
  function addItem(product){
    // product: { id, name, price, imageId, qty }
    const state = load();
  ...
  function inc(id){ const s = load(); const i = findIndex(s.items, id); if (i>=0){ s.items[i].qty++; save(s); render(); } }
  function dec(id){ const s = load(); const i = findIndex(s.items, id); if (i>=0){ s.items[i].qty = Math.max(1, s.items[i].qty-1); save(s); render(); } }
  function remove(id){ const s = load(); const i = findIndex(s.items, id); if (i>=0){ s.items.splice(i,1); save(s); render(); } }
  ...
  function render(){
    ...
    if (!body || !sub) return;
    const state = load();
    if (!state.items.length){ body.innerHTML = '<p>Seu carrinho está vazio.</p>'; sub.textContent = '0,00'; return; }
  ...
  function init(){ render(); attach(); }

  window.Cart = { addItem, inc, dec, remove, init, render };
})();
